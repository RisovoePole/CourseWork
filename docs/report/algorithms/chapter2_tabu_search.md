# II.3. Поиск с запретами (Tabu Search)
...
### Оптимизация размера окрестности поиска
...
*Рисунок II.2. Влияние размера окрестности на итоговое качество расписания (medium)*

```mermaid
xychart-beta
    title "Рисунок II.2. Влияние neighborhood_size на качество решения"
    x-axis ["Размер 10", "Размер 25", "Размер 50"]
    y-axis "Средний soft-штраф" 0.75 --> 1.15
    bar [1.08, 0.80, 0.84]
...
*Рисунок II.8. Сходимость TS: динамика soft-штрафов (medium)*

```mermaid
xychart-beta
    title "Рисунок II.8. Сходимость TS (дискретные итерации)"
    x-axis "Итерации" [0, 25, 50, 75, 100, 125, 150, 175]
...
При анализе параметров метода мы оценили влияние размера окрестности (`neighborhood_size`) на итоговую точность и временные затраты. Рост размера окрестности с 10 до 25 итеративно улучшает показатели штрафа. Дальнейшее увеличение до 50 не приводит к качественным сдвигам, формируя «плато» с незначительной деградацией результата при существенном росте вычислительных затрат. Таким образом, мы определяем значение 25 как оптимальное для данной архитектуры задачи.

*Рисунок II.13. Зависимость качества решения от размера окрестности*

```mermaid
xychart-beta
    title "Рисунок II.13. Влияние neighborhood_size на штраф"
    x-axis ["Размер 10", "Размер 25", "Размер 50"]
